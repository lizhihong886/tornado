...
class FileField(Field):
    ...
    def __init__(self,error_msg_dict=None,required=True):
        error_msg={'required':'请选择一个文件','valid':'数字格式错误'}
        if error_msg_dict:
            error_msg.update(error_msg_dict)
...
class Form(object):
    ...
    def validata(self,request,depth=10,pre_key=""):
    ...
    def __valid(self,form_obj,request,depth,pre_key):
        """
        验证用户表单请求的数据
        :param form_obj:Form 对象（Form派生的对象）
        :param request: Http请求上下文（用于从请求中获取用户提交的值）
        :param depth:对Form内容的深度的支持 
        :param pre_key: Html中的name属性的前缀(多层Form时，内部递归时设置，无需理会)
        :return:是否验证通过，True：成功，False:失败
        """
        depth-=1
        if depth<0:
            return None
...
class ListForm(object):
    ...
    def validate(self,request):
        name_list=request.request.arguments.key()+request.request.files.keys()
        index=0
        flag=False
        while True:
            pre_key="[%s]"%index
